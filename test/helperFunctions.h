...
inline std::shared_ptr<yap::Model> dkkp(int pdg_D, std::vector<int> fsps)
{
    ...
    std::transform(fsps.begin(), fsps.end(), std::back_inserter(FSP), [&F](int pdg){return yap::FinalStateParticle::create(F[pdg]);});
    ...
    auto D = yap::DecayingParticle::create(F[pdg_D], radial_size);
